...
let numberHumanOfficers = humanOfficers.count

// Q1: What percentage of humans are female?

let humanFemaleOfficers = humanOfficers.filter {femaleOfficers.contains($0)}
let numberHumanFemaleOfficers = humanFemaleOfficers.count
let percentageFemaleOfficers: Float = Float(numberHumanFemaleOfficers) / Float(numberHumanOfficers)

// Q2: How many first officers are human?

let humanFirstOfficers = firstOfficers.filter {humanOfficers.contains($0.key)}
let numberHumanFirstOfficers = humanFirstOfficers.count
...
let OfficerCaptainArray = captains.map {Officer(name: $0.key, ship: $0.value)}
OfficerCaptainArray


// Q5: Alphabetize the previous list by the captain's name
let CaptainsSorted = OfficerCaptainArray.sorted(by: {$0.name < $1.name})
CaptainsSorted
/*:
Group 2
*/
...
let shortNameVulcanFirstOfficers: Int = VulcanFirstOfficers.filter{$0.name.count < 5}.count
